...
Now that I am in the US, I find that the infrastructure question looks very different here. EV stations exist not just for passenger cars but for freight trucks moving goods across the country. I was curious what that infrastructure actually looks like on the ground. That question led me to the Texas Triangle, and to two specific questions:
...
This analysis was fast because the data was ready-made. It took less than an hour to gather the data. Charger locations came from the NREL AFDC, a federally maintained registry with a clean API. Highway geometry came from the US Census TIGER/Line dataset via a single `pygris` function call. Both are structured and built for exactly this kind of spatial query.

Running the same analysis on, say, the Trans-Java corridor, the toll road from Merak to Banyuwangi, would not be as straightforward. Indonesia has no equivalent to the NREL AFDC. The most complete source for SPKLU locations is [Pom Listrik](https://pomlistrik.id), a community-built map, alongside PLN's mobile app. Neither exposes a bulk API. Getting a corridor-wide charger list would mean scraping or manual export, with no guarantee the data is complete or current. For road geometry, the highway network would come from [OpenStreetMap via the Overpass API](https://overpass-api.de), which works but is slow and the accuracy and recency depend on how recently contributors have updated the toll road tags.

The result is that the same question requires a few API calls in the US and a multi-step scraping process in Indonesia. The evidence base for infrastructure planning is harder to build and slower to update when the underlying data lives in community maps and private apps rather than government databases.

Indonesia's EV challenge is partly about whether PLN can build enough chargers, and partly about whether anyone can systematically measure where the gaps are in the first place.
