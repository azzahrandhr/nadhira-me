---
title: "Does Temperature Shape Economic Growth? Replicating Burke, Hsiang & Miguel (2015)"
...
**Burke, Hsiang, and Miguel (2015)** published "Global non-linear effect of temperature on economic production" in *Nature* and offered one of the most compelling empirical answers to a deceptively simple question: does temperature actually affect how much countries grow? 
...
This post is a replication of their core analysis. I reproduce their main findings using the same dataset, then add a robustness check using an alternative growth specification. By the end, I want to make two things clear: (1) the temperature-GDP relationship is robust and well-identified, and (2) countries like Indonesia, which are already sitting in the hottest part of the curve, are facing compounding risks from further warming.
...
-   **The identification strategy relies on within-country temperature variation over time.** Country and year fixed effects remove time-invariant confounders (geography, institutions, culture) and global shocks (recessions, pandemics). What remains is the within-country, year-to-year variation in temperature — plausibly exogenous to economic decisions.

-   **The damage from future warming falls disproportionately on poor, hot countries.** Rich countries cluster near the optimal temperature. Poor countries in the tropics are already on the declining side — additional warming only makes things worse.
...
- **Growth rates** range from severe contractions to rapid expansions; 50% of observations fall between roughly −0.3% and 4% annually — reasonable for typical country-year variation.
...
I estimate the model using `feols` from the `fixest` package, which handles high-dimensional fixed effects efficiently. The main specification includes country ($\alpha_i$) and year ($\delta_t$) fixed effects.
...
::: callout-note
**Why fixed effects support a causal interpretation** — but only under assumptions.

Country fixed effects absorb time-invariant confounders: geography, institutions, culture, and historical development patterns. Year fixed effects absorb global shocks: recessions, oil prices, global technology shocks. What remains is within-country, within-year temperature variation — year-to-year deviations from a country's typical climate. This variation is plausibly exogenous to economic decisions.

The key assumptions: (1) strict exogeneity, meaning that temperature shocks aren't caused by GDP changes; (2) no time-varying omitted confounders within countries; (3) no spillovers across borders. These are defensible but not guaranteed.
:::
...
This is equivalent to the WDI measure for small growth rates (within ±10%), but avoids reliance on the WDI's data processing choices.
...
This replication confirms the main findings of Burke, Hsiang, and Miguel (2015): temperature has a significant, nonlinear effect on GDP growth, with an inverted-U shape peaking around 13–17°C depending on specification. The effect is identified from plausibly exogenous within-country temperature variation and is robust to alternative growth measures.
